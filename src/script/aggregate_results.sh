...
echo "aggregating prompting and prefix+prompting results"
for base in "roberta-base" "roberta-large"; do
  for length in 10 15 20; do
    for task in "SST-2" "cr" "RTE" "mr"; do
      ...
      python3 aggregate_few_shot_results.py "../checkpoints/finetuning/${base}/prefix+prompting/pre_seq_len${length}/${task}/" "${task}" "16" "13,21,42,87,100"
    done
  done
done
